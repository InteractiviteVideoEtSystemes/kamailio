...
static int cd_set_ruri(struct sip_msg * msg, str * newruri)
{
    struct action  act;
    ...
    act.val[0].u.string = (char *) pkg_malloc(newruri->len+1);
    memcpy(act.val[0].u.string, newruri->s, newruri->len);
    act.val[0].u.string[newruri->len] = '\0';
    act.type = SET_URI_T;
    init_run_actions_ctx(&h);
    if (do_action(&h, &act, msg)<0)
    {
        LM_ERR("do action failed\n");
	pkg_free(act.val[0].u.string);
        return -1;
    }
    pkg_free(act.val[0].u.string);
    return 1;
...
int cd_lookup(struct sip_msg* _msg, char* _table, char* _group)
{
	...
	valsToMatch[key_count].type=DB1_STR;
...
str _get_group(struct sip_uri *from_uri)
{
	...
	if(RES_ROW_N(result) == 0) {
		...
		switch(RES_ROWS(result)[0].values[0].type) {
			...
			default:
				LM_ERR("Unknown type of DB new_uri column %d\n", RES_ROWS(result)[0].values[0].type);
				returnValue.len=-1;
		}
	}
