...
static int nh_refresh_contact_keepalive(sip_msg_t *msg, char * str1, char * str2);
...
static cmd_export_t cmds[] = {
	...
	{"is_rfc1918",         (cmd_function)is_rfc1918_f,           1,
		fixup_spve_null, 0,
		ANY_ROUTE },
	{"refresh_contact_keepalive", (cmd_function) nh_refresh_contact_keepalive,           1,
		domain_fixup, 0,
...
static void update_contact_received_addr(urecord_t *r, udomain_t * d, ucontact_t *uc, str rcv_uri)
{
	ucontact_info_t ci;

	LM_INFO("refresh_keepalive: [%.*s] -> [%.*s]\n", uc->received.len, uc->received.s, rcv_uri.len, rcv_uri.s);

	ci.ruid = uc->ruid;
	ci.c = &uc->c;
	ci.received = rcv_uri;	
	ci.path = &uc->path;
	ci.expires = uc->expires;
	ci.q = uc->q;
	ci.callid = &uc->callid;
	ci.cseq = uc->cseq;
	ci.flags = uc->flags;
	ci.cflags = uc->cflags;
	ci.user_agent = &uc->user_agent;
	ci.sock = uc->sock;
	ci.methods = uc->methods;
	ci.instance = uc->instance;
	ci.reg_id = uc->reg_id;
	ci.tcpconn_id = uc->tcpconn_id;
#ifdef WITH_XAVP
       	ci.xavp = uc->xavp;         /*!< per contact xavps */
#endif
	if (ul.update_ucontact( r, uc, &ci ) < 0)
	{
		LM_ERR("refresh_keepalive: failed to update rcv addr.\n");
	}
}

/* IVES function that enables to refresh contact keepalive using OPTIONS sent by UAS */
static int nh_refresh_contact_keepalive(sip_msg_t *msg, char * str1, char * str2)
{
	contact_t *c;
	ucontact_t *uc;
	urecord_t *r;
	udomain_t * d = (udomain_t*) str1;
	struct sip_uri uri;
	int nbct = 0, i;
	...
	if(nh_keepalive_timeout<=0)
 	...
 	/* obtain contact */
        if (get_contact_uri(msg, &uri, &c) == -1)
	{
		LM_ERR("refresh_keepalive: Failed to parse contact URI\n");
                return -1;
	}

	if ( create_rcv_uri(&rcv_uri, msg) < 0 )
	{
		LM_ERR("refresh_keepalive: Failed to get received addr\n");
                return -3;
	}

	aorhash =  ul.get_aorhash( &uri.user );

	ul.lock_udomain(d, &uri.user);
	if ( ul.get_urecord( d, &uri.user, &r ) != 0 )
	{
		LM_WARN("refresh_keepalive: no contact registered for %.*s.\n", uri.user.len,  uri.user.s );
		ul.unlock_udomain(d, &uri.user);
		return -2;
	}
	
	/* save all the contact to refresh in a table */
	uc = r->contacts;
	while (uc != NULL)
	{
		/* Select contact to refresh */
		if ( strncasecmp( c->uri.s, uc->c.s, uc->c.len) == 0 && msg->rcv.proto == PROTO_UDP )
		{
			strncpy( ruids[nbct], uc->ruid.s, uc->ruid.len );
			ruids[nbct][uc->ruid.len] = 0;
			nbct++;
			/* Fix recevied addr for NAT with short UDP assotiation */
			if ( ! STR_EQ(rcv_uri, uc->received) )
			{
				LM_INFO("refresh_keepalive: rcv'ed address of contact [%.*s] has changed.\n", uc->c.len, uc->c.s);
				LM_INFO("refresh_keepalive: [%.*s] -> [%.*s]\n", uc->received.len, uc->received.s, rcv_uri.len, rcv_uri.s);
				update_contact_received_addr(r, d, uc, rcv_uri);
			}
		}
		uc = uc->next;
	}
	...
	/* now refresh - unloacked (otherwise deadlock) */
	for (i =0; i< nbct; i++)
	{
		str ruid = { ruids[i], strlen( ruids[i] ) };
		ul.refresh_keepalive( aorhash, &ruid );
	}

	LM_DBG("refresh_keepalive: refreshed %d contact(s) of AOR %.*s\n", nbct,  uri.user.len,  uri.user.s );

	return (nbct > 0) ? nbct : -1 ;
}
