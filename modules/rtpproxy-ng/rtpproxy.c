...
static int is_dst_in_internal_net0(struct sip_msg *msg, char *str1, char *str2);
static int is_uri_in_internal_net1(struct sip_msg *msg, char *str1, char *str2);
static int is_from_internal_net0(struct sip_msg *msg, char *str1, char *str2);
...
/* internal (vpn) network def */
static str internal_net_str = { NULL, 0 };

static struct net internal_net;
...
static cmd_export_t cmds[] = {
	...
	{"rtpproxy_manage",	(cmd_function)rtpproxy_manage2,     2,
		...
		ANY_ROUTE},
	{"is_from_internal_net", (cmd_function) is_from_internal_net0,     0,
		0, 0,
		ANY_ROUTE},
	{"is_uri_in_internal_net", (cmd_function) is_uri_in_internal_net1,     1,
		fixup_spve_null, fixup_free_spve_null,
		ANY_ROUTE},
	  
	{"is_dst_in_internal_net", (cmd_function) is_dst_in_internal_net0,     0,
		0, 0,
		ANY_ROUTE},
...
static param_export_t params[] = {
	...
	{"internal_network",      STR_PARAM, &internal_net_str.s },
	{0, 0, 0}
};
...
mod_init(void)
{
	...
	internal_net_str.len = strlen(internal_net_str.s);
	if ( internal_net_str.s != NULL && internal_net_str.len > 0 )
	{
	    if ( mk_net_str( &internal_net, &internal_net_str) )
	    {
		LM_ERR("Invalid internal network addr [%.*s] Check internal_network parameter\n",
			internal_net_str.len, internal_net_str.s);
		LM_ERR("Ex: internal_network=10.0.0.0/255.0.0.0\n");
		return -1;
	    }	    
	    else
	    {
		LM_NOTICE("Configured %.*s as internal network.\n",
			internal_net_str.len, internal_net_str.s);
	    }
	}
	else
	{
	    LM_INFO("No internal network configured.\n");
	    memset( &internal_net, 0, sizeof(internal_net) );
	}
	...
	enum rtpp_operation op, const char *flags_str, const str *force_addr, str *body_out)
{
	...
	if (op == OP_OFFER || op == OP_ANSWER) {
		...
		if ( is_from_internal_net0(msg, NULL, NULL) == 1)
		{
		    LM_INFO("msg is coming from VPN\n");
		    bencode_list_add_string(direction, "fromvpn");
		}
		
		if ( is_dst_in_internal_net0(msg, NULL, NULL) == 1)
		{
		    LM_INFO("msg is going to VPN\n");
		    bencode_list_add_string(direction, "tovpn");
		}
...
static int is_from_internal_net0(struct sip_msg *msg, char *str1, char *str2)
{
    if ( internal_net_str.s)
    {
	return is_from_internal_net(msg, &internal_net);
    }
    else
    {
        LM_DBG("rtpproxy: is_from_internal_net0: no internal network defined.\n");
	return -1;
    }
}
...
static int is_uri_in_internal_net1(struct sip_msg *msg, char *str1, char *str2)
{
	...
	get_str_fparam(&uri_str, msg, (fparam_t *) str1);
	return is_uri_in_internal_net_str(&uri_str, &internal_net);
    }
    else
    {
        LM_DBG("rtpproxy: is_uri_in_internal_net1: no internal network defined.\n");
	return -1;
    }
}
...
static int is_dst_in_internal_net0(struct sip_msg *msg, char *str1, char *str2)
{
	...
	if ( msg->first_line.type == SIP_REQUEST )
	{
	    if (msg->dst_uri.len > 0)
	        ret = is_uri_in_internal_net_str(&msg->dst_uri, &internal_net);
	    else if (msg->new_uri.len > 0)
	        ret = is_uri_in_internal_net_str(&msg->new_uri, &internal_net);
	    else 
	        ret = is_uri_in_internal_net_str(&msg->first_line.u.request.uri, &internal_net);
	}
	else
	{
	    // This is a reply. Use the via header
	    str * uri;

	    // First, make sure it is parsed
	    if (parse_headers( msg, HDR_VIA2_F, 0 )==-1 || (msg->via2==0) || (msg->via2->error!=PARSE_OK))
	    {
		LM_ERR("Failed to parse or get via2. We don't know where the message should go.\n");
		return -1;
	    }
	
	    // Use received
	    if ( msg->via2->received )
	        uri = &(msg->via2->received->value);
	    else
	        uri = &(msg->via2->host);
	    ret = is_uri_in_internal_net_str(uri, &internal_net);
	}
	return ret;
    }
    else
