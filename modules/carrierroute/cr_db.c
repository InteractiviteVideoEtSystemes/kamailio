...
int load_route_data_db(struct route_data_t * rd) {
	...
	do {
		...
		for (i = 0; i < RES_ROW_N(res); ++i) {
			...
			if ( row->values[COL_DOMAIN].type == DB1_INT )
			{
				domain_id = row->values[COL_DOMAIN].val.int_val;
			}
			else if ( row->values[COL_DOMAIN].type == DB1_STRING )
			{
				tmp_domain_name.s = (char *) row->values[COL_DOMAIN].val.string_val;
				tmp_domain_name.len = strlen(tmp_domain_name.s);
				domain_id = map_name2id( rd->domain_map, rd->domain_num, &tmp_domain_name );
				if (domain_id < 0 )
				{
					LM_WARN("cr: Unknown domain %s for route prefix %s.\n",
						tmp_domain_name.s, tmp_scan_prefix.s);
					continue;
				}
				else
				{
					LM_DBG("cr: domain %s passed as str -> id %d.\n", tmp_domain_name.s, domain_id );
				}
			}
			else
			{
				LM_ERR("carrierroute: unsupported value type for domain column. Must be string or int.\n");
				return -1;
			}
